ChipsCard from simui-lovelace. Use via `window.SimUI.ChipsCard` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<SimuiProvider>` (full provider chain in README.md — components read theme/i18n from that context). Keywords: chips, status, strip, pills, glanceable, header, row.
...
config: { entities?: string[]; type?: string }
}
```
