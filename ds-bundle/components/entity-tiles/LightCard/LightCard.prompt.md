LightCard from simui-lovelace. Use via `window.SimUI.LightCard` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<SimuiProvider>` (full provider chain in README.md — components read theme/i18n from that context). Keywords: light, bulb, brightness, toggle, dimmer, tile.
